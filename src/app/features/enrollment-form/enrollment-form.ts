...
export class EnrollmentFormComponent {
  ...
  // It is similar to constructor injection in .NET (like inject ILogger in aC# class).
  // The "private" keyword means only this class can access it.
  private fb = inject(FormBuilder);
  // A signal to track whether the form was submitted (for showing a successmessage)
  ...
  form = this.fb.nonNullable.group({
    ...
    term: ['Fall 2026', Validators.required], // Pre-filled with a defaultterm
  ...
  submit() {
    if (this.form.valid) {
      ...
      // IMPORTANT: Do NOT use .value here. If any field is disabled, .valuesilently
      // drops that field from the object. getRawValue() always includeseverything.
      const payload = this.form.getRawValue();
      console.log('Enrollment payload:', payload);
      this.submitted.set(true);
    } else {
      // markAllAsTouched() forces Angular to show validation errors on everyfield.
      // Without this call, Angular only shows errors on fields the user hasclicked on.
